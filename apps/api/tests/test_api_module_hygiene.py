"""API 包的结构卫生检查：每个模块不允许出现"用到但未定义/未导入"的名字。
...
这条测试对 `endless_task/api/**` 做静态分析，成本毫秒级，搬家/重构期间能立刻
指出漏导入。见 docs/product-improvements/05-code-health/01-monolith-audit.md。
"""
...
import endless_task.api

API_ROOT = Path(endless_task.api.__file__).parent
...
class ApiModuleHygieneTest(unittest.TestCase):
    def test_no_undefined_names_in_api_package(self) -> None:
        problems: dict[str, list[str]] = {}
        for path in sorted(API_ROOT.rglob("*.py")):
            missing = _check_module(path)
            if missing:
                problems[str(path.relative_to(API_ROOT))] = missing
        self.assertEqual(
            problems,
            {},
