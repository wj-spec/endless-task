"""应用装配（从 app.py 搬出；依赖构造与默认值零改动）。

`_build_container` 是原来的"装配大函数"（约 1 580 行）：按 settings 造出仓储、
服务、工具注册表、运行时网关等，最后塞进 `AppContainer`。它不注册路由、也不
处理请求，所以可以从 `app.py` 独立出去。`app.py` 仍 re-export
`AppContainer` / `_build_container`，既有导入路径不变。
"""
...
from endless_task.delegation import CancelAgentLegacyTool, CoordinatorDelegationHandler, QueryAgentLegacyTool, SpawnAgentLegacyTool
from endless_task.domain.models import ConversationKind, KnowledgeScope, MemoryKind
...
from endless_task.knowledge import CitationFeedbackProvider, EmbeddingIndexer, KnowledgeLifecycleService, KnowledgeProposalService, build_embedder
from endless_task.knowledge.embeddings import Embedder
from endless_task.mcp_runtime import McpManager
from endless_task.memory import MemoryConflictService, MemoryConsolidationService, MemoryForgettingService, MemoryProposalService, MemoryReflectionService, ReflectionTerminalObserver, UserProfileService
from endless_task.proposals.budget import ProposalBudget
from endless_task.runtime import FakeProvider, OpenAICompatibleProvider, P0ContextBuilder, RuntimeEventBroker, UnconfiguredProvider
...
from endless_task.runtime_v2 import AgentRunExecutor, MemoryScope, RunStatus, RuntimeV2SessionGateway, ToolExecutionLimits, UnattendedToolApprovalGate
...
from endless_task.runtime_v2.run_trajectory import FanoutTraceObserver, RunTrajectoryExporter, default_journal_reader
from endless_task.runtime_v2.trace_observer import LedgerTraceObserver, default_run_reader, default_usage_exists
from endless_task.skills import Skill, SkillService, build_available_skills_prompt
from endless_task.storage import Database, GENERAL_SCOPE_KEY, SqliteArtifactProposalRepository, SqliteArtifactRepository, SqliteChatRepository, SqliteHubEventRepository, SqliteKnowledgeProposalRepository, SqliteKnowledgeRepository, SqliteMemoryConsolidationRepository, SqliteMemoryProposalRepository, SqliteMemoryReflectionRepository, SqliteMemoryRepository, SqliteNotificationRepository, SqlitePreferencesRepository, SqliteReminderRepository, SqliteResponseFeedbackRepository, SqliteRetrievalEventRepository, SqliteRuntimeV2MemoryRepository, SqliteRuntimeV2Repository, SqliteTaskProposalRepository, SqliteTaskRepository, SqliteTaskRunRepository, SqliteTextFileRepository, SqliteUndoJournalRepository, SqliteUserProfileRepository, SqliteVecSearch, SqliteWorkspaceRepository
from endless_task.storage.provider_secret_store import ProviderSecretStore
from endless_task.storage.sqlite_mcp_server_repository import SqliteMcpServerRepository
from endless_task.storage.sqlite_provider_profile_repository import SqliteProviderProfileRepository
from endless_task.storage.sqlite_skill_override_repository import SqliteSkillOverrideRepository
from endless_task.storage.sqlite_skill_provenance_repository import SqliteSkillProvenanceRepository
from endless_task.storage.sqlite_skill_usage_repository import SqliteSkillUsageRepository
from endless_task.tasks import TaskNotificationService, TaskProposalService, TaskRunReviewService, TaskScheduler, TaskWorker
from endless_task.tooling import ToolApprovalMode, ToolCall, ToolCallStatus, ToolError, ToolRegistry
from endless_task.workspace_runtime import DeleteWorkspaceFileTool, EditWorkspaceFileTool, EffectLog, ListWorkspaceDirTool, ManageWorkspacePathsTool, ReadSkillFileTool, ReadWorkspaceFileTool, RunShellTool, WorkspaceResolver, WorkspaceSearchTool, WorkspaceUndoService, WriteWorkspaceFileTool
from endless_task.workspace_runtime.artifact_store import ArtifactFileStore
from endless_task.workspace_runtime.skill_install_tool import SkillInstallTool, SkillInstallTrustPolicy
